...
export default function Nav({ settings, forceSolid = false, tourBar }: NavProps) {
    ...
    <>
      ...
      >
        <ul>
          {LINKS.map(({ id, label }) => (
            <li key={id}>
              <a href={`/#${id}`} onClick={closeAnd(onHome ? () => document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' }) : undefined)}>
                {label}
              </a>
            </li>
          ))}
