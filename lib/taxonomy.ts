...
export function travelTypeValueFromLabel(label?: string): string | undefined {
  ...
  const found = TRAVEL_TYPES.find((t) => t.label.toLowerCase() === label.toLowerCase())
  return found?.value
}
...
export function buildMonthOptions(count = 7, from: Date = new Date()): MonthOption[] {
  const opts: MonthOption[] = []
  for (let i = 0; i < count; i++) {
    const d = new Date(from.getFullYear(), from.getMonth() + i, 1)
    const value = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`
    opts.push({ value, label: `${MONTH_NAMES_LONG[d.getMonth()]} ${d.getFullYear()}` })
  }
  return opts
}
 ...
 */
export function tourDuration(startDate?: string, endDate?: string): number | undefined {
